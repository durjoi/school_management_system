...
class Marks:
    ...
    '''
        Create a new marks for student
    '''
    ...
    def get_class_report(self, class_id, subject_id=None):
        ...
        pipeline = [
            ...
            {
                "$group": {
                    "_id": {"class_id": "$class_id", "subject_id": "$subject_id"},
            ...
            {"$group": {
                ...
                "total_students": {"$sum": "$total_count"}
            }},
            {"$unwind": "$subjects"},
            {"$project": {
                "_id": 0,
                "subject_name": "$subjects.subject_name",
                "pass_count": "$subjects.pass_count",
                "fail_count": "$subjects.fail_count",
                "total_count": "$subjects.total_count",
                "pass_percentage": {
                    "$multiply": [{"$divide": ["$subjects.pass_count", "$subjects.total_count"]}, 100]
                }
            }},
            {"$group": {
                "_id": "$subject_name",
                ...
                "pass_percentage": {"$avg": {"$cond": [
                    {"$gte": [{"$divide": ["$pass_count", "$total_count"]}, 0.4]}, {"$divide": ["$pass_count", "$total_count"]}, 0]
                }}
            }},
            {"$project": {
                "_id": 0,
